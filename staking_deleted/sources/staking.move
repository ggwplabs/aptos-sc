module staking::staking {
    ...
    public fun get_current_epoch_start_time(staking_addr: address): u64 acquires StakingInfo {
        ...
        let (current_epoch, _) = get_epoch_by_time(staking_info.start_time, now, staking_info.epoch_period);
        let start_time = staking_info.start_time;
        let epoch_period = staking_info.epoch_period;

        let spent_time = (current_epoch - 1) * epoch_period;
